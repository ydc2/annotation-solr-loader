...
class AnnotationSolrLoader
  ...
  def initialize
    ...
    #@notag = File.open("notag", "w")
    # read tag adjustments into a hash
    #@tagAdjustments = {'#ground_gold'=>'#ground_gold #ground_goldmine'}
    @tagAdjustments = JSON.parse(open("importData/tagAdjustments").read)
    p "tagAdjustments = #{@tagAdjustments.inspect}"
  ...
  def load_all_annotations()
    ...
    puts 'tags = ' + open( getTagsSolrMappingsUrl).read
    @all_tags = Set.new
    solr_data = Array.new
  ...
  def create_solr_record(annotation, manifest_lookup)
    ...
    folio_pre, folio_num, folio_suf = ''
    string = record[:canvas_label_t]
    record[:folio_t] = ''
    if !string.nil?
      matches = string.match(/^(.*?)(\d+)(\D*)$/) if !string.nil?
      if !matches.nil?
        folio_pre ,folio_num ,folio_suf = string.match(/^(.*?)(\d+)(\D*)$/).captures
      else
        folio_num = string
      end
      folio_num = folio_num.rjust(5, '0')
      record[:folio_t] = folio_pre
      record[:folio_t] += folio_num
      record[:folio_t] += folio_suf if !folio_suf.nil?
    end

    #record[:folio_t] = getFolio record[:canvas_label_t] if !record[:canvas_label_t].nil?
    #p 'folio_t = ' + record[:folio_t]
    ...
    if (record[:text_t])
      ...
      # [jrl] add call to replace/append tags here; check against an input list like this:
      # #red:#vermilion               replace red with vermilion
      # #red:#crimson #scarlet        replace red with scarlet and crimson
      # #red: #red #scarlet #crimson  append scarlet and crimson to red
      record[:text_t] = checkTagAdjustments(record[:text_t])
      ...
      record[:text_t].scan(/\#\w*/).each { |m|
        unless record[:project_id_t] == 'gratian'
          ...
          if @jsonTagCat[thisTag].nil?
            if record['unclassified_t'].nil?
              record['unclassified_t'] = thisTag
            else
              record['unclassified_t'] = record['unclassified_t'].concat(thisTag)
            end
            #p "unclassified: #{m}"
            #@notag.puts "unclassified: #{thisTag}"
          end
          if !@jsonTagCat[thisTag].nil?
  ...
  def add_metadata(record,manifest)
    ...
    if (metadata)
      ...
      metadata.each { |item|
        ...
        # add solr facet for 'origin' metadata
        if item['label'].downcase == 'origin'
          record['origin_t'] = item['value']
        end
      }
    end
  end
  ...
  def writeSolrFields(solrField, solrValue, record)
    ...
    solrValueArray.each do |solrValueSeg|
      ...
      begin
        ...
        puts "solr field: #{solrField} solrvalue: #{solrValue}"
        puts ''
      end
    end
  end
  ...
  # deprecated
  def getFolio canvas_label_t
    oldKey = canvas_label_t.to_s
    keyNum = oldKey.scan(/\d+/).first.to_s
    if !keyNum.empty?
  ...
  def checkTagAdjustments(text_t)
    text_out = ""
    textArr = text_t.split(" ")
    textArr.each { |word|
      if !@tagAdjustments[word].nil?
        text_out = text_out + @tagAdjustments[word] + " "
      else
